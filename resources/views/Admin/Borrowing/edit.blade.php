...
<div class="p-8 max-w-lg">
    ...
    <form action="{{ route('admin.borrowings.update', $borrowing) }}" method="POST" class="bg-white p-6 rounded-lg shadow">
        @csrf
        @method('PUT')
        ...
        <div class="mb-4">
            ...
            <input type="date" name="borrow_date" value="{{ $borrowing->borrow_date }}" class="w-full border rounded px-4 py-2 focus:ring-2 focus:ring-blue-500" required>
        </div>

        <div class="mb-4">
            <label class="block mb-2 font-semibold">Tanggal Kembali</label>
            <input type="date" name="return_date" value="{{ $borrowing->return_date }}" class="w-full border rounded px-4 py-2 focus:ring-2 focus:ring-blue-500" required>
        </div>
        ...
        <div class="mt-6 flex justify-end gap-2">
            <a href="{{ route(auth()->user()->role->name . '.borrowings.index') }}" class="px-4 py-2 border rounded hover:bg-gray-50">Batal</a>
